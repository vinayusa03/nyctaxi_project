...
# Lets analyze which all columns has null values and see if we can drop any rows
yelTriRawDF.select([F.count(F.when(F.col(c).isNull(),1)).alias(c) for c in yelTriRawDF.columns]).display()

# COMMAND ----------

# Lets check the summary of the DF
# yelTriRawDF.summary().display()
...
# Lets unpersist DF
yelTriRawDF.unpersist()

# COMMAND ----------

# Lets filter rows which are out of 6 Months window
yelFilteredDF = yelTriRawDF.filter(
    (F.col('tpep_pickup_datetime') >= '2024-12-01') & (F.col('tpep_pickup_datetime') < '2025-06-01') &
    ((F.col('tpep_dropoff_datetime') >= '2024-12-01')) & ((F.col('tpep_dropoff_datetime') < '2025-06-01'))
    )
...
yelFilteredDF.filter(F.col('tpep_dropoff_datetime') == '2024-12-18T07:52:40.000').display()
...
).write.mode('overwrite').saveAsTable('nyctaxi.`02_silver`.yellow_trips_cleansed')
...
# MAGIC SELECT Count(*) FROM nyctaxi.`02_silver`.`yellow_trips_cleansed`;